...
from keras_models.attention_model import VisualizeAttentionMap, \
    get_dense_model, get_model_with_additive_attention
...
experiment_name = "With additive attention"

f.write(experiment_name)

# Generate the dataset
trainX, trainY, testX, testY = get_transformed_data(dataset_number_to_load=1)

n_samples = trainX.shape[0]
n_inputs = trainX.shape[1] # number of features
output_size = 7
# #model_RNN= get_dense_model(num_of_output_classes=output_size,input_dim=n_inputs, lr=0.01)
...
model_attention= get_model_with_additive_attention(num_of_output_classes=output_classes, input_dim=n_features, lr=0.01)
...
accuracy_file_name = f'{configuration.attention_experiments_results_plots_path}\\additive_attention_acc.png'
loss_file_name = f'{configuration.attention_experiments_results_plots_path}\\additive_attention_loss.png'

plot_validation_and_train_acc(file_name=accuracy_file_name, title="Accuracy model with additive attention", history=attention_history)
plot_validation_and_train_loss(file_name=loss_file_name, title="Loss model with additive attention", history=attention_history)
...
# Print error
content_train= f"Model with attention: train loss={train_loss:.2f}, train acc:{train_acc:.2f}\n"
content_test = f"Model with attention: test loss={test_loss:.2f}, test acc:{test_acc:.2f}\n"
